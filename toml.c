...
TOML_CONST TomlErr *toml_err(void) { return &g_err; }
...
TOML_STATIC void toml_err_set_literal(TomlErrCode code,
                                      TOML_CONST char *message) {
  assert(g_err.code == TOML_OK);
  g_err.code = code;
...
TOML_STATIC size_t toml_roundup_pow_of_two_size_t(size_t x) {
  size_t v = x;
  v--;
  v |= v >> 1;
  v |= v >> 2;
...
TOML_STATIC void toml_string_expand_if_necessary(TomlString *self,
                                                 size_t len_to_add) {
  if (self->len + len_to_add + 1 > self->_capacity) {
    size_t new_capacity =
        toml_roundup_pow_of_two_size_t(self->len + len_to_add + 1);
...
void toml_string_append_nstr(TomlString *self, TOML_CONST char *str,
                             size_t len) {
  toml_string_expand_if_necessary(self, len);
...
TomlValue *toml_table_get_by_string(TOML_CONST TomlTable *self,
                                    TOML_CONST TomlString *key) {
...
TomlValue *toml_table_getn(TOML_CONST TomlTable *self, TOML_CONST char *key,
                           size_t key_len) {
...
TomlTable *toml_table_get_as_table(TOML_CONST TomlTable *self,
                                   TOML_CONST char *key) {
...
TomlArray *toml_table_get_as_array(TOML_CONST TomlTable *self,
                                   TOML_CONST char *key) {
  TomlValue *v = toml_table_get(self, key);
  assert(v != NULL);
...
int toml_table_get_as_boolean(TOML_CONST TomlTable *self,
                              TOML_CONST char *key) {
  TomlValue *v = toml_table_get(self, key);
  assert(v != NULL);
...
TomlKeyValue *toml_table_iter_get(TomlTableIter *self) { return self->_keyval; }
...
*/
  if (self->ptr + 3 <= self->end && (strncmp(self->ptr, "nan", 3) == 0 ||
                                     strncmp(self->ptr, "inf", 3) == 0)) {
    type = 'f';
  }

  if (self->ptr + 4 <= self->end && (strncmp(self->ptr, "+nan", 4) == 0 ||
...
*/
  ...
  while (self->ptr < self->end) {
    if (*self->ptr == '+' || *self->ptr == '-') {
      if (last_char == 0 ||
          ((last_char == 'e' || last_char == 'E') && !has_exp)) {
        if (last_char != 0) {
          type = 'f';
          has_exp = TOML_TRUE;
        }
      ...
      if (type == 'i') {
        ...
        toml_err_set(TOML_ERR_SYNTAX, "%s:%d:%d: invalid float", self->filename,
...
TomlValue *toml_parse_value(TomlParser *self) {
  ...
  if (strncmp(self->ptr, "\"\"\"", 3) == 0) {
  ...
  } else if (isdigit(ch) || ch == '+' || ch == '-' || ch == '.' || ch == 'n' ||
             ch == 'i') {
    ...
    toml_err_set(TOML_ERR_SYNTAX, "%s:%d:%d: unexpected token", self->filename,
                 self->lineno, self->colno);
  }
  ...
  if (is_array) {
    ...
    for (i = 0; i < key_path->len; i++) {
      ...
      if (t == NULL) {
        ...
        if (t->type == TOML_ARRAY) {
          real_table =
              t->value.array->elements[t->value.array->len - 1]->value.table;
...
TomlErrCode toml_parse_table(TomlParser *self, TomlTable *table) {
  ...
  if (key_path->len == 0) {
    toml_err_set(TOML_ERR_SYNTAX, "%s:%d:%d: empty table name", self->filename,
                 self->lineno, self->colno);
    goto cleanup;
  }
  ...
  if (self->ptr < self->end && *self->ptr != '\n') {
    toml_err_set(TOML_ERR_SYNTAX, "%s:%d:%d: new line expected", self->filename,
                 self->lineno, self->colno);
    goto cleanup;
  }
...
TomlTable *toml_load_file_filename(FILE *file, TOML_CONST char *filename) {
  ...
  do {
    ...
    if (ferror(file)) {
      toml_err_set(TOML_ERR_OS, "Error when reading %s [errno %d: %s]",
...
TomlTable *toml_load_filename(TOML_CONST char *filename) {
  ...
  f = fopen(filename, "r");
  if (f == NULL) {
    toml_err_set(TOML_ERR_OS, "Cannot open file %s [errno %d: %s]", filename,
